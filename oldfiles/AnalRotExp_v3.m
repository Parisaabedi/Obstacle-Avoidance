...
for i = 1 : 18
    for j = 1:3 
        ...
        if valid == 1           
            ...
            %dxrhr = max(trajc(:,1,3+c*3,1,obs(j,1),1,i)) - max(trajc(:,1,1+c*3,1,obs(j,1),1,i));
            %dxlhr = max(trajc(:,1,3+c*3,1,obs(j,2),2,i)) - max(trajc(:,1,1+c*3,1,obs(j,2),2,i));
            %dxrhl = max(trajc(:,1,2+c*3,1,obs(j,1),1,i)) - max(trajc(:,1,1+c*3,1,obs(j,1),1,i));
            %dxlhl = max(trajc(:,1,2+c*3,1,obs(j,2),2,i)) - max(trajc(:,1,1+c*3,1,obs(j,2),2,i));
            
            % calculate deltaX for different directions and conditions (Use max curvature)
            dxrhr = DfromO(3+c*3,1,obs(j,1),1,i) - DfromO(1+c*3,1,obs(j,1),1,i);
            dxlhr = DfromO(3+c*3,1,obs(j,2),2,i) - DfromO(1+c*3,1,obs(j,2),2,i);
            dxrhl = DfromO(2+c*3,1,obs(j,1),1,i) - DfromO(1+c*3,1,obs(j,1),1,i);
            dxlhl = DfromO(2+c*3,1,obs(j,2),2,i) - DfromO(1+c*3,1,obs(j,2),2,i);
...
for j = 1 : 6
    ...
    figure(101); hold on; scatter(repmat(xs(j),1,18),ddata)
    ms(j,1) = nanmean(ddata); ms(j,2) = nanstd(ddata);
end
figure(101); hold on; plot(xs,ms(:,1))
...
%% save the results
save('RotExp3.mat','alfabeta','alfabetap')
