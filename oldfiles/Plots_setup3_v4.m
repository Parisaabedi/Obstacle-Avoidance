...
%     close all
    ...
    % for center, rightward, and most rightward obstacles
    for jj = 1:5
        figure(100); subplot(1,5,jj); hold on
        for ii = 1:3            
            for d = 1 : 2                
                tempx = trajcc(:,1,ii+1*3,1,jj,d) - 1*18.875;
                ys = trajcc(:,2,ii+1*3,1,jj,d);
    ...
    figname ='MTraj2-VF-obstacle1,2,3 '; 
    savefig(gcf,figname) 
    saveas(gcf,figname,'tiff')
    close all

    % for the selected obstacles
    obs = [ 5 1 ; 4 2; 3 3 ];
    dir = [0 1 ; 0 1 ; 0 1]+1;
    
    for jj = 1:5 % obstacles
        figure(100); subplot(1,5,6-jj); hold on
        for ii = 1:3 % HR conditions
            for d = 1:2
                tempx = trajcc(:,1,ii+0*3,1,obs(jj-2,d),dir(jj-2,d)) - 1*18.875;
                ys = trajcc(:,2,ii+0*3,1,obs(jj-2,d),dir(jj-2,d));
                uE=tempx + trajcc(:,1,ii+0*3,2,obs(jj-2,d),dir(jj-2,d))./sqrt(18);
                lE=tempx - trajcc(:,1,ii+0*3,2,obs(jj-2,d),dir(jj-2,d))./sqrt(18);
                xP=[lE;flipud(uE)];
                yP=[ys;flipud(ys)];
                patch(xP,yP,color(ii,:),'FaceAlpha',0.3,'EdgeColor','none');
                plot(tempx,ys,color(ii,:),'Linewidth',2, 'LineStyle',':')
                scatter([obstaclepos(obs(jj-2,d),1)-18.8750 0 0],[30-obstaclepos(obs(jj-2,d),2) 9 29],'ro')
            end
        end
    end
...
%         saveas(gcf,figname,'tiff')        
    ...
    for c = 1 : 2
        ...
        for i= 1 : 18
            plot(1:2,[mvars(i,1+(c-1)*3,1),mean(mvars(i,(2:3)+(c-1)*3,1))],':k','LineWidth',0.5)
        end
        errorbar(1:2,[mvar(1,1+(c-1)*3) mean(mvar(1,(2:3)+(c-1)*3))],...
            [mvar(2,1+(c-1)*3) mean(mvar(2,(2:3)+(c-1)*3))],'-k','LineWidth',2.5)
        xlabel('Head Rotations (deg)'); ylabel('Movement variability')
        set(gca,'XTick',1:2)
        set(gca, 'XTickLabels',{'HR = 0' , 'HR != 0'})
        xlim([0.5 2.5])
    end     
    ...
    figure(100);hold on
    for i = 1 : 18
        plot(1:3,expansions(:,i),':k','LineWidth',0.5)
    end
    errorbar(1:3,mean(expansions'),std(expansions'),'-k','LineWidth',2.5)
    xlabel('Obstacle difficualty'); ylabel('Expansion value')
    set(gca,'XTick',1:3)
    set(gca, 'XTickLabels',{'Easy' , 'Medium', 'Hard'})
    xlim([0.5 3.5])

    figname = 'Expansions-averaged and individuals';
    savefig(gcf,figname)
    saveas(gcf,figname,'tiff')
    close all
    ...
    for i = [2 1 3]
        ...
        l = reshape(RD(i,:,2,:),5,18);l = flipud(l);
        pr = r.*100./(r+l); 
        errorbar(1:5,nanmedian(pr'),nanstd(pr'),'Color',colors{i},'LineStyle',':','LineWidth',2.5);        
    ...
    for i = [2 1 3]
        ...
        l = reshape(RD(i+3,:,2,:),5,18);l = flipud(l);
        pr = r.*100./(r+l); 
        errorbar(1:5,nanmean(pr'),nanstd(pr'),'Color',colors{i},'LineStyle','-','LineWidth',2.5);
    ...
    figname = strcat('LR-MDir-averaged and individuals');
    savefig(gcf,figname)
    saveas(gcf,figname,'tiff')
    close all
...
%% Plot distance from the obstacle
    ...
    for i = 1:18
        plot(1:2,[NCc(1,1,i),mean(NCc(2:3,1,i))],':k','LineWidth',0.5)
    end
    tt = zeros(6,2);
    for i = 1:6
        tt(i,:) = mean(NCc(i,1,:),3);
    ...
    errorbar(1:2,[tt(1,1) mean(tt(2:3,1))],[tt(1,2) mean(tt(2:3,2))],':k','LineWidth',2.5)
    xlabel('Head Rotations (deg)'); ylabel('Number of collisions')
    set(gca,'XTick',1:2)
    set(gca, 'XTickLabels',{'HR = 0' , 'HR != 0'})
    xlim([0.5,2.5])
    ...
    for i = 1:18
        plot(1:2,[NCc(1+3,1,i),mean(NCc((2:3)+3,1,i))],':k','LineWidth',0.5)
    end
    tt = zeros(6,2);
    for i = 1:6
        tt(i,:) = mean(NCc(i,1,:),3);
    ...
    errorbar(1:2,[tt(1+3,1) mean(tt((2:3)+3,1))],[tt(1,2) mean(tt(2:3,2))],'-k','LineWidth',2.5)
    xlabel('Head Rotations (deg)'); ylabel('Number of collisions')
    set(gca,'XTick',1:2)
    set(gca, 'XTickLabels',{'HR = 0' , 'HR != 0'})
    xlim([0.5,2.5])

    figname = 'Collisions-averaged and individuals'; 
    savefig(gcf,figname)
    saveas(gcf,figname,'tiff')
    close all
